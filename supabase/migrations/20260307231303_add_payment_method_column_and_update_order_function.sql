/*
  # Add payment_method column to user_orders

  1. Modified Tables
    - `user_orders`
      - Added `payment_method` (text, default 'cashOrSwish')
        - Stores how the customer chose to pay: 'cashOrSwish', 'points', or 'payAtStore'

  2. Modified Functions
    - `create_order_atomic` - Updated to accept and store `p_payment_method` parameter

  3. Important Notes
    - Existing orders default to 'cashOrSwish' unless they have paid_with_points=true,
      in which case they are backfilled to 'points'
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'user_orders' AND column_name = 'payment_method'
  ) THEN
    ALTER TABLE user_orders ADD COLUMN payment_method text NOT NULL DEFAULT 'cashOrSwish';
  END IF;
END $$;

UPDATE user_orders
SET payment_method = 'points'
WHERE paid_with_points = true AND payment_method = 'cashOrSwish';
...
BEGIN
  ...
  INSERT INTO user_orders (user_id, total, contact_email, contact_phone, shipping_address, delivery_instructions, paid_with_points, points_amount, status, payment_method)
  VALUES (p_user_id, p_total, p_contact_email, p_contact_phone, p_shipping_address, p_delivery_instructions, p_paid_with_points, p_points_amount, 'active', COALESCE(p_payment_method, 'cashOrSwish'))
