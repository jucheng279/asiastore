/*
  # Fix wrap-around weekly schedule in create_order_atomic

  1. Changes
    - Update the auto-schedule check inside create_order_atomic to handle
      wrap-around windows where close day is earlier than open day
      (e.g., open Friday, close Monday -- a weekend window)
    - Previously, the guard assumed close > open which would always reject
      orders when the schedule wraps around the week boundary

  2. Logic
    - When close_minutes > open_minutes (normal range): reject if now < open OR now >= close
    - When close_minutes <= open_minutes (wrap-around): reject if now < open AND now >= close

  3. Important Notes
    - Only the schedule comparison logic changes; all other order logic is untouched
    - The full function is recreated with CREATE OR REPLACE
*/
