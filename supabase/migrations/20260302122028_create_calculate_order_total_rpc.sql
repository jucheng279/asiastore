/*
  # Create server-side order total calculation RPC

  1. New Function
    - `calculate_order_total(p_items jsonb, p_pay_with_points boolean)`
      - Accepts a JSON array of {productId, quantity} objects
      - Looks up current prices from products, expiry_items, and flash_sale_items tables
      - Calculates subtotal, shipping, tax, points discount, and total
      - Returns a JSON object with the verified totals
      - Prevents client-side price manipulation

  2. Security
    - Function runs as SECURITY DEFINER to access all product tables
    - Execution granted to anon and authenticated roles
    - All price lookups happen server-side for tamper-proof calculations

  3. Business Rules
    - Tax rate: 25%
    - Free shipping threshold: 35 (currency units)
    - Shipping fee: 5 (currency units)
    - Points discount rate: 5% of total before discount
*/
...
DECLARE
  ...
  v_points_discount_rate numeric := 0.05;
